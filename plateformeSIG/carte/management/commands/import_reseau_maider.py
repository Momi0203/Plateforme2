"""
Commande : python manage.py import_reseau_maider [--replace]

Module INDÉPENDANT d'import du réseau hydrographique « ouvrage de tête » du
bassin Maïder, lu depuis :
  plateformeSIG/plateformeSIG/static/resaux hydrographique ouvrage en tete/maider.shp

Insère les tronçons (LineString) dans le modèle carte.ReseauOuvrageTeteMaider.
Forme du modèle : grid_code + geometrie (aucune clé étrangère).

NB : le shapefile maider.shp n'est pas encore présent dans le dossier — déposez-le
...
from carte.models import ReseauOuvrageTeteMaider
...
MODELE        = ReseauOuvrageTeteMaider
SHP_CANDIDATS = ["maider", "Maider", "madier", "Madier", "maïder", "Maïder"]   # noms .shp essayés
...
class Command(BaseCommand):
    help = "Importe le réseau hydrographique (ouvrage de tête) du bassin Maïder."
