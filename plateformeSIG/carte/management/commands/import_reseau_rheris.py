"""
Commande : python manage.py import_reseau_rheris [--replace]

Module INDÉPENDANT d'import du réseau hydrographique « ouvrage de tête » du
bassin Rhéris, lu depuis :
  plateformeSIG/plateformeSIG/static/resaux hydrographique ouvrage en tete/rheris.shp

Insère les tronçons (LineString) dans le modèle carte.ReseauOuvrageTeteRheris.
Forme du modèle : grid_code + geometrie (aucune clé étrangère).

NB : le shapefile rheris.shp n'est pas encore présent dans le dossier — déposez-le
...
from carte.models import ReseauOuvrageTeteRheris
...
MODELE        = ReseauOuvrageTeteRheris
SHP_CANDIDATS = ["rheris", "Rheris", "rhéris", "Rhéris"]   # noms de fichier .shp essayés
...
class Command(BaseCommand):
    help = "Importe le réseau hydrographique (ouvrage de tête) du bassin Rhéris."
